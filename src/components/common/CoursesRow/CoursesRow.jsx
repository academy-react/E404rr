import CourseCard from "../CourseCard/CourseCardForLanding";
...
const CoursesRow = () => {
  ...
  const getList = async () => {
    const courses = await getLastCoursesList();
  ...
  return (
      <>
          ...
          {lastCoursesList.map((item , index) => {
              return <CourseCard key={index} tumbImageAddress={item.tumbImageAddress} dissLikeCount={item.dissLikeCount} title={item.title} id={item.courseId} desc={item.desc} teacherName={item.teacherName} cost={item.cost} likeCount={item.likeCount} /> ;
