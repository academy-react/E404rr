...
import { AddDeleteLikeForCourses, AddDeleteLikeForNews, AddDisLikeForCoursesComment, AddLikeForCoursesComment, AddLikeForNewsComment } from '../../../core/services/api/CourseAddCourseLike';

const CommentNews = ({title , describe , id , dissLikeCount , LikeCount}) => {
...
//   const UserId = useParams().id;
  ...
  const handleClick = async () => {
    if (selectedSvg !== 1) {
      ...
      const user = await AddLikeForNewsComment(id);
    } else {
      setSelectedSvg(null);
      setCount(count - 1);
      // const user2 = await AddDeleteLikeForNews();
    }
  };
  ...
  const handleClick2 = async () => {
    if (selectedSvg !== 2) {
      ...
      const user = await AddDisLikeForCoursesComment(id);
    } else {
      setSelectedSvg(null);
      setCount2(count2 - 1);
      // const user2 = await AddDeleteLikeForNews();
  ...
  return (
      ...
      <div className="w-[80%]">
        ...
        <div className="border border-transparent mt-3">
              <div className="inline-block">
                ...
                <p className="inline-block"> {count + LikeCount}</p>
              </div>

              <div className="inline-block mr-[20px]">
                <p className="inline-block">{  count2 + dissLikeCount}</p>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  fill="currentColor"
    ...
    {showReplies && (
        ...
        {data.map((item , index) => {
           return(
            <> 
                  <ReplayComentNews likeCount={item.likeCount} dissLikeCount={item.dissLikeCount} author={item.author} describe={item.describe} key={index} title={item.title}  id={item.id}/>
            </>
           )
        })}
